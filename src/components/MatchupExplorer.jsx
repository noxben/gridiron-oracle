...
function enrichPlayer(p, allPlayers) {
  const gsisId =
    p.gsisId ??
    ESPN_TO_GSIS[String(p.espn_id ?? '')] ??
    null;

  const nflData = gsisId
    ? PLAYER_BY_GSIS_ID?.[gsisId]
    : null;
  ...
  const {
    ...
    snapScore,
    redZoneScore,
  } = computeCompositeRating(nflData, allPlayers);

  const projectedPts =
    projectPoints(nflData, compositeRating, 0);

  const replacement =
    getReplacementLevel(nflData.position, 12);
...
// Get starting lineup
//
// ESPN provides the actual lineup, but this league has 9 starting slots:
// QB, RB, RB, WR, WR, TE, FLEX, K, DST.
//
// If ESPN gives us all 9 starters, preserve them exactly.
// If ESPN gives us only 8, preserve those 8 and fill the missing FLEX
// with the highest-projected available RB/WR/TE.
// ---------------------------------------------------------------------------

function getStartingLineup(enrichedRoster) {
  const espnStarters = enrichedRoster.filter(
    p => !p.on_bench && !p.on_ir
  );

  console.log(
    '[Matchup] ESPN starters:',
    espnStarters.map(p => ({
      name: p.name,
      position: p.position,
  ...
  const selected = [...espnStarters];

  /*
   * ESPN is currently giving us 8 explicit starters because
   * FLEX isn't represented as a separate starter slot.
   * Add the best eligible bench RB/WR/TE as FLEX.
   */
  if (selected.length < 9) {
    const selectedIds = new Set(
      selected.map(p =>
        String(p.espn_id ?? p.gsisId)
      )
    );

    const flexPlayer = enrichedRoster
      .filter(
        p =>
          !p.on_ir &&
          p.on_bench &&
    ...
    if (flexPlayer) {
      ...
      console.log('[Matchup] FLEX selected:', {
        name: flexPlayer.name,
        position: flexPlayer.position,
        projectedPts: flexPlayer.projectedPts,
      });
    }
  }

  return sortBySlot(selected);
}
...
function sortBySlot(players) {
  ...
  return players
    ...
    .sort((a, b) => {
      const slotA =
        a.player.lineup_slot ??
        a.player.lineupSlot ??
        a.player.position ??
        'BENCH';

      const slotB =
        b.player.lineup_slot ??
        b.player.lineupSlot ??
        b.player.position ??
        'BENCH';

      const priorityA =
        slotPriority[slotA] ?? 7;

      const priorityB =
        slotPriority[slotB] ?? 7;
...
function MatchupRow({
  ...
  const myPts =
    myPlayer?.projectedPts ?? 0;

  const oppPts =
    oppPlayer?.projectedPts ?? 0;

  const myWins = myPts > oppPts;
  const tied =
    Math.abs(myPts - oppPts) < 0.5;

  const mySelected =
    selectedId &&
    (
      myPlayer?.gsisId === selectedId ||
      myPlayer?.espn_id === selectedId
    );

  const oppSelected =
    selectedId &&
    (
      oppPlayer?.gsisId === selectedId ||
      oppPlayer?.espn_id === selectedId
    );
...
// ---------------------------------------------------------------------------
// Compact opponent bench row
// ---------------------------------------------------------------------------

function OpponentBenchRow({
  player,
  onSelect,
  selectedId,
}) {
  ...
  const isSelected =
    selectedId &&
    (
      player?.gsisId === selectedId ||
      player?.espn_id === selectedId
    );
...
function PlayerDetailPanel({
  ...
  const stats = [
    {
      label: 'Projected pts',
      val: player.projectedPts,
      fmt: v => v?.toFixed(1),
    },
    {
      label: 'Composite rating',
      val: player.compositeRating,
      fmt: v => v?.toFixed(0),
    },
    {
      label: 'Season avg pts',
      val:
        player.season_avg_pts ??
        player.avg_points,
      fmt: v => v?.toFixed(1),
    },
    {
      label: 'EPA / play',
      val: player.epa_per_play,
      fmt: v => v?.toFixed(3),
    },
    {
      label: 'Snap %',
...
export default function MatchupExplorer({
  ...
  const matchup =
    teamData?.matchup ?? null;

  const myTeam =
    teamData?.myTeam ?? null;

  const oppTeamId =
    matchup?.opp_team_id ?? null;

  const oppTeamName =
    matchup?.opp_team_name ??
    'Opponent';

  const oppTeam =
    (ALL_TEAMS ?? []).find(
      t => t.team_id === oppTeamId
    );
  ...
  // Benches
  //
  // Explicitly exclude players selected into the starting lineup.
  // This also handles the FLEX player added by getStartingLineup().
  // -------------------------------------------------------------------------

  const myStarterIds = useMemo(
    () =>
      new Set(
        myStarters.map(p =>
          String(
